...
def setup(bot: commands.Bot):
    ...
    @discord.app_commands.describe(
        ...
        owner="Discord user ID (leave empty for shared key, or your own ID for personal key)"
    )
    async def torn_key_add(
        ...
        try:
            ...
            if validation.get("valid"):
                ...
                embed.add_field(
                    name="Permissions",
                    value=", ".join(validation.get("permissions", [])[:10]) or "None",
                    inline=False
                )
    ...
    async def torn_key_check(interaction: discord.Interaction, key_alias: str):
        ...
        try:
            ...
            embed.add_field(
                name="Permissions",
                value=", ".join(key_meta.get("permissions", [])[:15]) or "None",
                inline=False
            )
            embed.add_field(
                name="Last Validated",
    ...
    async def torn_key_validate(interaction: discord.Interaction, key_alias: str):
        ...
        try:
            ...
            if validation.get("valid"):
                ...
                embed.add_field(
                    name="Permissions",
                    value=", ".join(validation.get("permissions", [])[:15]) or "None",
                    inline=False
                )
                embed.add_field(
                    name="Last Validated",
